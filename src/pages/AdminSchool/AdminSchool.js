...
import { CreateSchoolAction, DeleteSchoolAction, GetListSchoolAction, UpdateSchoolAction } from "../../redux/action/SchoolAction";

export default function AdminShcool() {
  const dispatch = useDispatch();
  const { arrSchool } = useSelector((root) => root.SchoolReducer);
  ...
  const [text, setText] = useState("Thêm mới trường");
  ...
  const saveProduct = async () => {
    ...
    if (product.description) {
      ...
      if (product.product_id !== "0") {
        ...
        const action = await UpdateSchoolAction(product.school_id, product);
        await dispatch(action);
        setProductDialog(false);
        toast.current.show({
          severity: "success",
          summary: "Thành công",
          detail: `Cập nhật trường ${product.name} thành công`,
          life: 8000,
          options: {
            className: "custom-margin-class",
          },
        });
        setText("Chỉnh sửa trường");
      } else {
        console.log(_product);
        const action = await CreateSchoolAction(product);
        await dispatch(action);
        toast.current.show({
          severity: "success",
          summary: "Thành công",
          detail: "Tạo  mới trường thành công",
          life: 3000,
        });
        setProductDialog(false);
      }
  ...
  const deleteProduct = async () => {
    const action = await DeleteSchoolAction(product.school_id);
    await dispatch(action);
    setDeleteProductDialog(false);
    setProduct(emptyProduct);
    toast.current.show({
      severity: "error",
      summary: "Thành công",
      detail: `Xóa trường ${product.name} thành công`,
      life: 3000,
      options: {
        style: {
          zIndex: 100,
      ...
      <div>
        ...
        <div className="card">
          ...
          <DataTable
            ...
            currentPageReportTemplate="Đang hiển thị {first} đến {last} trong tổng số {totalRecords} trường"
        ...
        >
          <div className="confirmation-content" style={{ margin: "20px 20px" }}>
            ...
            {product && (
              <span>
                Bạn có chắc chắn muốn xóa trường <b>{product.name}</b>?
              </span>
            )}
          </div>
        </Dialog>
